...
export const getProductById = async (req, res, next) => {
  try {
    ...
    if (!response) res.status(404).json({ msg: `Producto ${pid} no encontrado` });
...
export const deleteProduct = async (req, res, next) => {
  try {
    ...
    if (!productToDelete) res.status(404).json({ msg: "No se pudo eliminar el producto" });
    else res.status(200).json({ msg: `Producto con ID: ${pid} eliminado` });
  } catch (error) {
    next(error.message);
  }
};
